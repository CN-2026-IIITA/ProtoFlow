...
const MAX_LOGS = 100;
const SOCKET_URL = (import.meta.env.VITE_BACKEND_WS_URL as string | undefined) ?? "ws://localhost:4317/ws";
...
export const SecurityLogs = () => {
    ...
    const toggleSev = (sev: EventSeverity) => setSeverityFilter(prev => (prev === sev ? "ALL" : sev));
    ...
    const exportLogs = async () => {
        try {
            ...
            const response = await fetch("http://localhost:4317/export-logs");
            const data = await response.json();

            if (data.success) {
                setExportMessage({
                    type: "success",
                    text: `Logs saved: ${data.filename} (${data.logsCount} events)`,
                });
    ...
    return (
        <div className="p-8 lg:p-12 animate-in fade-in duration-500">
                ...
                <div className="flex items-center gap-6">
                    ...
                    <div className="flex items-center gap-4 text-xs font-mono">
                        <span className="flex items-center gap-1.5 text-primary">
                            <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse"></span> SYSTEM_LIVE
                        </span>
            ...
            <div className="p-8 flex flex-col gap-8">
                ...
                <div className="bg-surface-container-low rounded-2xl p-4 flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            id="btn-export-logs"
                            onClick={exportLogs}
                            disabled={exportLoading || activeLogs.length === 0}
                            className="flex items-center gap-2 px-4 py-2 bg-primary/15 rounded-xl text-xs font-medium hover:bg-primary/25 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                        >
                            <span className="material-symbols-outlined text-sm">
                                {exportLoading ? "hourglass_top" : "file_download"}
                            </span>
                ...
                {exportMessage && (
                    <div
                        className={`p-3 rounded-xl text-xs font-medium animate-in fade-in duration-300 ${
                    ...
                    <div className="bg-surface-container-low p-6 rounded-2xl flex flex-col gap-2">
                        ...
                        <div className="w-full h-1 bg-surface-container-highest rounded-full mt-2">
                            <div
                                ...
                                style={{
                                    width: `${Math.min((criticalCount / Math.max(activeLogs.length, 1)) * 100, 100)}%`,
                                }}
                            />
                        </div>
                    </div>
                    ...
                    <div className="bg-surface-container-low p-6 rounded-2xl flex flex-col gap-2">
                        ...
                        <div className="w-full h-1 bg-surface-container-highest rounded-full mt-2">
                            <div
                                ...
                                style={{
                                    width: `${Math.min((warningCount / Math.max(activeLogs.length, 1)) * 100, 100)}%`,
                                }}
                            />
                        </div>
                    </div>
